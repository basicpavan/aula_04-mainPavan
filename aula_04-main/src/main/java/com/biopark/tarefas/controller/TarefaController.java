...
public class TarefaController {
    ...
    public String listar(@RequestParam(required = false) String filtro, Model model) {
        ...
        // 1. Busca todas as tarefas para calcular os contadores (Tarefa 3)
        List<Tarefa> todas = tarefaService.listarTodas();
        long total = todas.size();
        long concluidas = todas.stream().filter(Tarefa::getConcluida).count();
        long pendentes = total - concluidas;

        // 2. Lógica do Filtro (Tarefa 1)
        List<Tarefa> tarefasFiltradas;
        if ("pendentes".equals(filtro)) {
            tarefasFiltradas = tarefaService.listarPendentes(); // Requer este método no Service
        } else if ("concluidas".equals(filtro)) {
            tarefasFiltradas = tarefaService.listarConcluidas(); // Requer este método no Service
        } else {
            tarefasFiltradas = todas;
            filtro = "todas"; // Padroniza o valor para a view
        }

        // 3. Enviando os dados e os contadores para a View (Model)
